...
class ROBOT:
	...
	def Send_Sensors(self,sim):
		...
		#	sim.Send_Light_Sensor(sensorID=4+l, objectID = l)

	def Send_Neurons(self,sim):

		for s in range(0,5): # 9):

			sim.Send_Sensor_Neuron(neuronID=s, sensorID=s )

		for m in range(0,8):

			sim.Send_Motor_Neuron(neuronID=5+m , jointID=m , tau=0.3)
	...
	def Send_Synapses(self,sim,wts):
		...
		for s in range(0,5): # 9):
			...
			for m in range(0,8):
				...
				sim.Send_Synapse(sourceNeuronID = s , targetNeuronID = 5+m , weight = wts[s,m,0] , finalWeight = wts[s,m,1] , transitionTime = wts[s,m,2])
